...
def scrap_113(url="https://sokchocl.go.kr/kr/member/active.do") -> ScrapResult:
...
def scrap_119(
    ...
    return ScrapResult(
        council_id="gangwon-yeongwol",
        council_type="LOCAL_COUNCIL",
        councilors=councilors,
    )


def scrap_120(url="https://cl.happy700.or.kr/kr/member/active.do") -> ScrapResult:
    ...
    return ScrapResult(
        council_id="gangwon-pyeongchang",
...
def scrap_122(
    ...
    for profile in soup.find_all("div", class_="person_pop_wrap"):
        # Extracting name of the councilor
        name_tag = profile.find("h2", class_="tit")
        name = name_tag.get_text(strip=True) if name_tag else "이름 정보 없음"

        # As the party information is not available, it is set to "정당 정보 없음"
        party = "정당 정보 없음"

        councilors.append(Councilor(name=name, party=party))

    return ScrapResult(
        council_id="gangwon-yanggu",
        council_type=CouncilType.LOCAL_COUNCIL.value,
        councilors=councilors,
    )
