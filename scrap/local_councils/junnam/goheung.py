...
def scrap_goheung(url = 'https://council.gc.go.kr/kr/member/active.do') -> ScrapResult:
    '''고흥군 페이지에서 의원 상세약력 스크랩
    ...
    for profile in mlist.find_all('li', recursive=False):
        ...
        name = info.find("h5").get_text(strip=True) if info.find("h5").get_text(strip=True) else "이름 정보 없음"
