...
def main():
    for folder_name in ["지선-당선", "지선-후보"]:
        for cluster_by in ["sdName", "wiwName"]:
            ...
            outdir = os.path.join(BASE_DIR, "output", f"age_all_{cluster_by}", folder_name)
            ...
            for d in os.listdir(datadir):
                ...
                cluster(df, year, 7, 'kmeans', cluster_by, outdir, font_name, folder_name)
                cluster(df, year, 7, 'equal', cluster_by, outdir, font_name, folder_name)
