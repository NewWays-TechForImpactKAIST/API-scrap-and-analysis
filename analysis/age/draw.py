...
def make_scatterplot(package):
    (outdir, total_population_count, year, df_age, n_clst, method, cluster_by, folder_name, colors, font_name) = package
        # 산점도로 클러스터링 결과를 시각화합니다.
    sns.set(style="whitegrid")  # Seaborn 스타일 설정 (선택적)
    plt.figure(figsize=(10, 6))  # 그림 크기 설정 (선택적)
    print(df_age)
    sns.scatterplot(
        data=df_age, x="age", y="area", palette="viridis"
    )
    ...
    for _, row in df_age.iterrows():
        ...
        age = row['age']
        print(age)
        plt.text(age, area, "{:.2f}".format(float(age)), fontsize=12, ha="right", fontname = font_name)
    plt.xlabel("나이", fontname = font_name)
    plt.ylabel("지역", fontname = font_name)
    plt.yticks(fontname=font_name)
    plt.title(f"{folder_name}자 나이 분포 ({year})  <총 {total_population_count}명>", fontname = font_name)
    # 그래프를 이미지 파일로 저장합니다.
    plt.savefig(os.path.join(outdir, method, f"clustering_result ({year}).png"), dpi=300)  # 파일 이름 및 해상도 설정 (선택적)
    plt.close()
...
def make_hist(package):
    (outdir, df, year, area, n_clst, method, cluster_by, folder_name, colors, font_name) = package
    ...
    if cluster_by == "sdName":
        plt.title(f"{area} {folder_name}자 나이 분포 ({year})  <총 {total_population_count}명>", fontname = font_name)
    elif cluster_by == "wiwName":
        sdName = df[df["wiwName"] == area]["sdName"].iloc[0]
        plt.title(f"{sdName} {area} {folder_name}자 나이 분포 ({year})  <총 {total_population_count}명>", fontname = font_name)
    else:
